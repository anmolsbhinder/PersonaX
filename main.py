...
criteria_list = ['TotalRevenue', 'TotalInvoices', 'ReturnedItems', 'BoughtItems',
...
for column in criteria_list:
    ...
    individual_criteria = criteria[['CustomerID', column]].copy()

    # Sort the new table by the column values in descending order
    individual_criteria = individual_criteria.sort_values(by=column, ascending=False)

    # Calculate the score based on the percentile rank of each row
    individual_criteria[f'Score_{column}'] = individual_criteria[column].rank(pct=True)
    individual_criteria[f'Score_{column}'] = individual_criteria[f'Score_{column}'] * 100
    individual_criteria.to_csv(f"{column}_Score.csv", index=False)
    # Merge the new table with the score_table on 'CustomerID'
    criteria_scores = pd.merge(criteria_scores, individual_criteria[['CustomerID', f'Score_{column}']], on='CustomerID', how='outer')
...
def create_new_score(criteria_scores, segment_scores, criteria_vector, type):
    ...
    # print(row)
    segment_master.loc[len(segment_master)] = row
    segment_master.to_csv('segment_master.csv', index=False)

    return calculate_scores(criteria_scores, segment_scores, criteria_vector, type)
...
def calculate_scores_for_all_segments(segment_scores):
    segment_master = pd.read_csv('segment_master.csv')
    ...
    for _, row in segment_master.iterrows():
        ...
        segment_scores = calculate_scores(criteria_scores, segment_scores, criteria_vector, type)
...
calculate_scores_for_all_segments(segment_scores)
